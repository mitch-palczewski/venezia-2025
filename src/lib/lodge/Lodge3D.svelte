<script lang="ts">
	...
	const grabSystem = useFPSGrab(({ grabbedObject, targetPosition, camera, delta }) => {
		grabbedObject.position.lerp(targetPosition, delta * 15);
		grabbedObject.quaternion.slerp(camera.quaternion, delta * 10);
	});
